...
# Load a pretrained NesT model with weights from official Google JAX impl (e.g., nest_base_jx)
def load_nest(weight, num_classes=1):
    """
    Loads a pre-trained NesT model and modifies its classification head for binary classification.
    Args:
        weight (str): Name of pretrained weights to load.
        num_classes (int): Number of output classes.
    Returns:
        nn.Module: NesT model.
    """
    model = nest_base_jx(pretrained=weight)
...
def test_model(model_name, dataloader, checkpoint_path, device, use_checkpoint=True):
    ...
    """
    # Load the model
    model = load_nest(True, num_classes=1)
